...
> **8.9 古法合规重构**：修正七处排盘与古法不一致之处（梅花起卦改农历月日、奇门拆补法改符头定元、时干甲遁防崩、六壬九宗门补全涉害/昴星/别责/八专并修正贼克优先序、太乙月支改月柱、紫微闰月归正、玄空流月改节气月逐月递减）；新增排盘审计日志（`runtime/paipan_audit.jsonl`）与控制台「排盘」标签页（奇门遁局/值符值使、六壬三传、八字日主旺衰、十盘断卦分实时展示）。
...
## 8.8 信号触发模式（默认）：三层 AI 分工 + 条件监听

8.8.0 起默认不再按固定周期全量重算，改为**信号触发**：由 AI 预设触发条件，监听循环按最小间隔轮询，**条件命中才下单**。

- **三层 AI 分工**：
  - **AI-1 断卦师**：接收 K 线行情上下文 + 完整古法盘面，逐项解读十项古法（奇门/六壬/太乙/易经/风水/八字/梅花/紫微/八卦/四柱），输出 `bias/confidence/reading`，单项失败用中性兜底，不阻断其余九项。
  - **AI-2 入场决策师**：综合 AI-1 十项解读、持仓与风控约束，确认是否买入、买入多少仓位，并把入场时机翻译成触发条件（`price_above/below`、相对买入价或现价的涨跌幅 `change_pct_above/below`、`rsi_above/below`、`volume_surge` 放量、`time_after` 到点）。
  - **AI-3 出场决策师**：以**买入成交均价为基准**设定卖出条件（止盈/止损/时间兜底，任一命中即平仓）。
- **古法择时**：AI-2 给出的入场条件生效前，系统扫描当天剩余时辰的排盘，取盘面最完整的档位作为**当日首次触发时刻**；未到该时刻只监听不入场。
- **最小间隔**：监听循环按 `runtime.trigger_poll_seconds`（默认 2 秒，OKX 批量 tickers 限频 20req/2s）轮询；RSI/放量指标带 60 秒缓存，避免 K 线接口超限频。
- **防反复唤醒**：AI-2 判定"今日不宜交易"冷却到次日 UTC 零点；AI 调用失败退避 10 分钟；入场条件超过 `trigger_max_wait_hours`（默认 24h）自动唤醒 AI-2 重新评估。
- **重启恢复**：已有持仓但无出场条件时，以持仓成本价为基准唤醒 AI-3 补设。
...
独立于 Web 控制台的只读监控大屏，用于桌面大屏实时盯盘：

```bash
python gufa_dashboard.py --config config.json
# 默认 http://127.0.0.1:8601
```

- **只读**：仅读取 `state_dir`（与主引擎共享同一 runtime 目录），不控制交易进程；
- **零依赖部署**：同机运行即可，无需额外数据库；
- 主要区块：KPI 六指标（权益/今日盈亏/回撤/持仓/布防/成交）、权益曲线（日/今日/3日/7日）、雷达初筛、布防触发、成交明细、关键事件日志；
- **排盘七式**（9.0 新增）：奇门九宫、六壬天盘四课三传、八字四柱、三易六爻（动爻高亮）、紫微十二宫环形、太乙计神、玄空飞星；
- 顶栏时辰卡：当前时辰干支与下一时辰倒计时；排盘按日/时柱缓存，每个时辰自动重排；
- 数据流：`/api/state`（轮询）+ `/api/stream`（SSE 实时推送）+ `/api/paipan/full`（十项排盘全量）。
...
默认运行状态写入 `./runtime/`。7.5.0 继续使用配置版本 `2`、状态版本 `4`；8.8.0 起状态版本升级为 `5`（新增 `triggers` 信号触发条件字段，v4 → v5 自动迁移补空）；凭据版本 `1`。旧状态版本 3 仍不自动迁移。
